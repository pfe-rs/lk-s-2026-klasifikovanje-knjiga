...
mlflow.set_experiment("5 klasa - parovi w2v")  
...
tokens = [str(text).split() for text in train_df['toc']]
...
with mlflow.start_run():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        prototypes = compute_prototypes(emb_net, train_df, labels, 'tfidf_vector', device)
        val_accuracy = compute_accuracy(emb_net, prototypes, val_df, 'tfidf_vector', device)
