...
klase_counts = df['Single_Label'].value_counts()
validne_klase = klase_counts[klase_counts >= 500].index
df= df[df['Single_Label'].isin(validne_klase)].reset_index(drop=True)
...
emb_net = EmbeddingNetwork(input_dim=600, embedding_dim=32)
...
with mlflow.start_run():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        test_vectors_np = np.stack(val_df['tfidf_vector'].values)
        test_vectors = torch.tensor(test_vectors_np, dtype=torch.float32).to(device)
        test_labels = val_df['Single_Label'].values
       ...
       # mlflow.log_metric("val_loss", epoch_val_loss, step=epoch)
        ...
        print(f"Epoch [{epoch+1:02d}/{NUM_EPOCHS}] -> Train Loss: {epoch_loss:.4f} - Val Accuracy: {accuracy:.4f}")
...
print("-" * 70)
for label, (pogodeno, ukupno, proceginat) in sorted_results:
