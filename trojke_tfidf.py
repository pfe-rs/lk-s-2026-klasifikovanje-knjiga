...
mlflow.set_experiment("26 klasa - trojke tfidf")
...
# df['toc'] = [lemmatize_text(doc) for doc in df['toc']]
...
def build_symmetric_triplets(source_df, labels, n_per_class):
    triplets_list = []

    for anchor_label in labels:
        df_anchor_class = source_df[source_df['LCSH_Label'] == anchor_label]
        other_labels = [l for l in labels if l != anchor_label]

        anchor_samples = df_anchor_class.sample(n=n_per_class, replace=True).reset_index(drop=True)
        positive_samples = df_anchor_class.sample(n=n_per_class, replace=True).reset_index(drop=True)

        negative_labels_choice = np.random.choice(other_labels, size=n_per_class)

        negative_rows = []
        for neg_label in other_labels:
            count_needed = int(np.sum(negative_labels_choice == neg_label))
            if count_needed == 0:
                continue
            neg_class_df = source_df[source_df['LCSH_Label'] == neg_label]
            sampled = neg_class_df.sample(n=count_needed, replace=True)
            negative_rows.append(sampled)
        negative_samples = pd.concat(negative_rows).sample(frac=1).reset_index(drop=True)
...
train_triplets_df = build_symmetric_triplets(train_df, labels, N_PER_CLASS_TRAIN)
val_triplets_df = build_symmetric_triplets(val_df, labels, N_PER_CLASS_VAL)
...
class EmbeddingNetwork(nn.Module):
    def __init__(self, input_dim=5000, embedding_dim=64):
...
emb_net = EmbeddingNetwork(input_dim=5000, embedding_dim=64)
...
def compute_prototypes(embedding_net, source_df, labels, device):
    """Racuna prototip (mean embedding) za svaku klasu na osnovu source_df."""
    prototypes = {}
    with torch.no_grad():
        for label in labels:
...
def compute_accuracy(embedding_net, prototypes, eval_df, device):
    """Klasifikuje svaki red eval_df na osnovu najblizeg prototipa (cosine distance)."""
...
# --- Finalna evaluacija na test setu (nakon treninga) ---
model.eval()
final_prototypes = compute_prototypes(emb_net, train_df, labels, device)

test_tfidf_matrix = tfidf.transform(test_df['toc'])
test_df['tfidf_vector'] = [np.array(vec).flatten() for vec in test_tfidf_matrix.toarray()]
